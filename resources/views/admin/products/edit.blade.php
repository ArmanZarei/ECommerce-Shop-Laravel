...
@push('scripts')
    ...
    <script>
        ...
        let oldImagesIds = @json($product->images->pluck('id')->toArray());
        ...
        $(document).ready(function () {
            ...
            $("#edit-form").on('submit', function (e) {
                ...
                for (let imageId of oldImagesIds)
                    $(this).append(`<input type=number name="old_image_ids[]" value="${imageId}" />`)
            });
            ...
            function createOldImage(imageId, imageUrl) {
                const elem = $(`<div class="col-3" style="margin-bottom: 25px">
                                <div class="d-inline-block" style="width: 200px; height: 200px">
                                    <div class="image-input-container">
                                        <i class="image-input-action delete-input fal fa-times" data-iid="${imageId}"></i>
                                        <img src="${imageUrl}">
                                    </div>
                                </div>
                            </div>`);
                elem.find('.delete-input').on('click', function () {
                    oldImagesIds.splice(oldImagesIds.indexOf($(this).data('iid')), 1);
                    elem.remove();
                });

                return elem;
            }
            Object.entries(@json($product->images->pluck('imageUrl', 'id')->toArray())).forEach(image => {
                productImagesContainer.append(createOldImage(image[0], image[1]));
            });
        });
    </script>
@endpush
        ...
        <div class="row">
            <div class="col-12">
                <div class="card border-0 bg-white rounded-2">
                    <h5 class="card-header bg-white">Edit product</h5>
                    <div class="card-body">
                        <x-form.form action="{{ route('admin.products.update', $product->id) }}" class="me-5 ms-5 mt-2" enctype="multipart/form-data" id="edit-form" method="PUT">
                            <div class="row mb-4">
                                <div class="col-3">
                                    <x-form.inputs.text name="name" :value="$product->name"/>
                                </div>
                                <div class="col-3">
                                    <x-form.inputs.select name="brand_id" label="Brand" :dataArray="$brands" :value="$product->brand_id"/>
                                </div>
                                <div class="col-4">
                                    <x-form.inputs.select name="tag_ids" label="Tags" :dataArray="$tags" :multiple="true" :value="$product->tags->pluck('id')->toArray()"/>
                                </div>
                                <div class="col-2 d-flex align-items-center justify-content-center">
                                    <x-form.inputs.switch name="is_active" label="Active" :checked="$product->is_active"/>
                                </div>
                            </div>
                            <div class="col-12 mb-4">
                                <x-form.inputs.textarea name="description" :value="$product->description" />
                            </div>
                            <div class="row mb-3">
                                <div class="col-3">
                                    <x-form.inputs.select name="category_id" label="Category" :dataArray="$categories" :value="$product->category_id" />
                                </div>
                            </div>
                                    ...
                                    @foreach($attributes->where('pivot.is_variation', false) as $attr)
                                        <div class="input-container col-3 mb-3">
                                            <x-form.inputs.text
                                                ...
                                                :label="$attr->name"
                                                :value="old('attributes') ? old('attributes')[$attr->id] : $product->attributes->where('id', $attr->id)->first()->pivot->value"
                                                :error-key='"attributes.$attr->id"'
                                            />
                                        </div>
                                    @endforeach
                                @endif
                            ...
                            <div id="product-variations" class="mb-4 {{ !$variationAttr ? "d-none" : '' }}">
                                ...
                                <div id="pvs-container">
                                    ...
                                    @if ($variationAttr && old('product_variations'))
                                        ...
                                        @endforeach
                                    @elseif ($variationAttr && $product->variations)
                                        @foreach($product->variations as $variation)
                                                <div class="row product-variation ps-5 relative mb-3">
                                                    <i class="fal fa-times remove-product-variation btn-light-danger"></i>
                                                    <div class="col-3">
                                                        <x-form.inputs.text :name='"old_product_variations[$variation->id][value]"' label="Value" :value="$variation->value" :error-key='"product_variations.$variation->id.value"' />
                                                    </div>
                                                    <div class="col-3">
                                                        <x-form.inputs.text :name='"old_product_variations[$variation->id][price]"' label="Price" :value="$variation->price" :error-key='"product_variations.$variation->id.price"' />
                                                    </div>
                                                    <div class="col-3">
                                                        <x-form.inputs.text :name='"old_product_variations[$variation->id][quantity]"' label="Quantity" :value="$variation->quantity" :error-key='"product_variations.$variation->id.quantity"' />
                                                    </div>
                                                </div>
                                        @endforeach
                                    @endif
                            ...
                            <div class="row mb-5 justify-content-between">
                                <div class="col-3 d-flex flex-column align-items-center">
                                    ...
                                    <div class="ms-3 mt-2" style="width: 300px; height: 300px">
                                        <x-form.inputs.image name="main_image" :value="$product->primary_image_url" />
                                    </div>
                                </div>
                                <div class="col-8">
