        ...
        <div class="row">
            <div class="col-12">
                <div class="card border-0 bg-white rounded-2">
                    ...
                    <div class="card-body">
                        <form class="custom-form me-5 ms-5 mt-2" action="{{ route('admin.attributes.update', $attribute->id) }}" method="POST">
                            @method('PUT')
                            @csrf
                            <div class="col-3 mb-3">
                                <div class="floating-label-container">
                                    <input type="text" name="name" autocomplete="off" placeholder=" " class="{{ $errors->has('name') ? 'has-error' : '' }}" value="{{ $attribute->name }}">
                                    <label>Name</label>
                                </div>
                                @if($errors->has('name'))
